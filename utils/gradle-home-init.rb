...
opts = Slop.parse do |o|
  ...
  o.bool '-n', '--noValidateProfile', "Skip loading and validation of settings.xml.template, supercedes x option"
  o.on '-h', '--help' do
    help(o)
    exit
  end
...
errors = false
if !opts[:noValidateProfile]
  errors = parseAndValidate(opts)
end
...
cmd = "./initGradleProfile.sh #{opts[:mvnSettingsParentDir]} #{opts[:mvnProfile]} #{opts[:mvnLocalRepo]} copySettingsXml"
puts "Running #{cmd}"
