plan piper::jenkins_create_jobs (
  ...
  #Patch Pipeline Job Builder
     ...
     $patch_builder_cmd = "ssh -o \"StrictHostKeyChecking=no\" -l ${targetall.config[ssh][user]}  -p ${targetall.vars[cli_port]} localhost  create-job PatchJobBuilder < <(cat ${patch_builder_template})"
    ...
    $generic_builder_cmd = "ssh -o \"StrictHostKeyChecking=no\" -l ${targetall.config[ssh][user]}  -p ${targetall.vars[cli_port]} localhost  create-job GenericPipelineJobBuilder < <(cat ${generic_builder_template})"
